...
export default class FunctionStyler {
  ...
  activate(state) {
    atom.workspace.observeTextEditors(editor => {
      ...
      atom.config.onDidChange("pokinano-functions-styler.lineColor", event => {
        atom.config.set("pokinano-functions-styler.lineColor", event.newValue);
        this.addHeaders(editor, markerLayer, fileGrammar);
      })
    });
  }
    ...
    lines.forEach((line, lineNumber) => {
      line.forEach(token => {
        if (
          ...
          const lineColor = atom.config.get("pokinano-functions-styler.lineColor").toHexString();
